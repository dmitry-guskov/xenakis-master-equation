...
def run(rng, npart=NPART, tau=3.0, alpha=ALPHA, tmax=TMAX, reg=REG, scale=0.20,
        sync=True):
    """Velocity-jump paths with reflecting register edges.

    `sync` is the thing that makes the sheet look like the sheet.  Xenakis works
    segment by segment across the whole orchestra: at each boundary EVERY part
    draws a new velocity, so between boundaries the ensemble is one clean family
    of 46 straight lines with a common origin in time -- and a family like that
    has an envelope.  Tumbling each part on its own Poisson clock smears the
    boundaries out and no coherent fold survives.

    tau is the segment length (sync) or the mean holding time (not sync); scale
    converts the sheet's velocity units into semitones per unit time."""
    ...
    while t < tmax:
        if sync:
            step = min(tau, tmax - t)              # a common segment boundary
        else:
            step = min(rng.exponential(tau / npart), tmax - t)
        x = x + v * step
        ...
        if t < tmax:
            if sync:
                v = velocities(rng, npart, alpha) * scale       # all parts redraw
            else:
...
def spreading(rng, tau, alpha=ALPHA, scale=0.20, n=20000, tmax=TMAX, sync=True):
    """<x^2>(t) against the velocity-jump prediction, for both conventions.

    Exponential holding times (the master equation's own Poisson structure):
        <x^2> = 2 sigma^2 tau^2 (t/tau - 1 + e^-t/tau),      D = sigma^2 tau
    Deterministic segments of length tau (what Xenakis actually draws, and what
    the figure uses), with t = n tau + r:
        <x^2> = sigma^2 (n tau^2 + r^2),                     D = sigma^2 tau / 2
    Both are sigma^2 t^2 for t << tau: the ballistic law, and the caustics, do
    not care which convention you use.  Only the long-time D differs, by two."""
    grid = np.linspace(0, tmax, 60)
    x = np.zeros(n)
    v = velocities(rng, n, alpha) * scale
    cur = np.zeros(n)
    nxt = np.full(n, tau) if sync else rng.exponential(tau, n)
    out = [0.0]
    for g in grid[1:]:
        while True:
            due = nxt < g
            ...
            nxt[due] += tau if sync else rng.exponential(tau, k)
        x += v * (g - cur); cur[:] = g
        out.append(np.mean(x ** 2))
    sig2 = (alpha * scale) ** 2 / 2.0
    if sync:
        nfull = np.floor(grid / tau + 1e-9)
        pred = sig2 * (nfull * tau ** 2 + (grid - nfull * tau) ** 2)
    else:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--tau-fast", type=float, default=None,
                    help="memory for the second panel (default: tau/8)")
    ...
    a = ap.parse_args()
    tau_b = a.tau
    tau_d = a.tau_fast if a.tau_fast else tau_b / 8.0
    ...
    sig2 = (ALPHA * 0.20) ** 2 / 2.0
    D = sig2 * tau_b / 2.0        # deterministic segments: D = sigma^2 tau / 2
    ...
    if a.check:
        for sync in (True, False):
            g, emp, pred = spreading(np.random.default_rng(a.seed + 1), tau_b,
                                     sync=sync)
            rel = np.abs(emp[2:] - pred[2:]) / pred[2:]
            law = ("deterministic segments, <x^2> = s^2 (n tau^2 + r^2)" if sync
                   else "exponential holding, <x^2> = 2 s^2 tau^2 (t/tau-1+e^-t/tau)")
            print(f"{law}\n   max rel. dev {rel.max():.3f}, mean {rel.mean():.3f}")
            for k in (2, len(g) // 2, len(g) - 1):
                print(f"   t={g[k]:5.2f}   empirical {emp[k]:8.2f}   "
                      f"predicted {pred[k]:8.2f}")
